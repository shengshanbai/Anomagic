...
def process_aitex_metadata(dataset_path):
    ...
    metadata = {
        ...
        "defect_types": [],
        "images": [],
        "objects": {},  # Added: statistics for objects and defects
    }
                ...
                if len(parts) >= 3:
                    ...
                    for mask_filename in get_possible_mask_names(filename):
                        if os.path.exists(os.path.join(mask_images_dir, mask_filename)):
                            ...
                            # Statistics for objects and defects
                            object_name = "fabric"
                            defect_name = aitex_defect_codes.get(defect_code, "Unknown")
...
def process_manta_metadata(dataset_path):
    """Create metadata for MANTA dataset."""
    ...
    metadata = {
        ...
        "defect_types": [],
        "images": [],
        "objects": {},  # Statistics for objects and defects
    }
    ...
    for category in os.listdir(dataset_path):
        ...
        # Traverse subcategories (e.g., test/train or specific subcategory names)
        for small_category in os.listdir(category_path):
            ...
            for defect_type in os.listdir(test_dir):
                ...
                # Traverse all images under defect type
                        ...
                        for mask_filename in get_possible_mask_names(filename):
                            ...
                            if os.path.exists(mask_path):
                                ...
                                metadata["images"].append({
                                    ...
                                    "category": category,
                                    "split": small_category  # test or train
                                })
...
def process_over1_metadata(dataset_path):
    """Create metadata for CYK dataset."""
    dataset_name = os.path.basename(dataset_path)
    metadata = {
        "name": dataset_name,
        "categories": [],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process anomaly detection datasets")
    parser.add_argument("--dataset_root", type=str, required=True, help="Root directory for datasets")
    parser.add_argument("--output_dir", type=str, default=None, help="Output directory for statistics reports")
    args = parser.parse_args()

    dataset_root = args.dataset_root
    output_dir = args.output_dir if args.output_dir else os.path.join(dataset_root, 'statistics_reports')
